...
const Hero = ({ getSearchValue }) => {
    ...
    return (
        <div className='bg-gradient-to-t from-[#FFFFFF] to-[#ffffff00] p-5 md:p-12 rounded-3xl text-center flex flex-col-reverse md:flex-col justify-between items-center gap-5 mb-20 border-white border-2 text-black'>
            ...
            <form onSubmit={e => getSearchValue(e, search)} className='flex items-center w-full justify-center gap-4'>
                <div className='w-8/12 md:w-6/12'>
                    <input value={search} onChange={e => setSearch(e.target.value)} className='bg-white border border-gray-400 w-full py-2 md:py-3 px-4 rounded-full' placeholder='search any doctor...' type="text" name="" id="" />
                </div>
                <div>
                    <button type='submit' className='bg-[#176AE5] px-6 py-3 rounded-full text-white font-medium text-xs md:text-base'>Search Now</button>
                </div>
            </form>
